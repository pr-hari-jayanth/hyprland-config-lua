...
if [[ "$1" == "menu" ]]; then
    ...
    THEME=$(printf "$THEMES" | rofi -dmenu -p "Theme" -mesg "Current: ${CURRENT}" -select "$CURRENT" \
        -theme-str 'window {width: 300px;} listview {lines: 4; spacing: 2px; padding: 4px 4px 0;} element {padding: 0;} element-text {padding: 10px 16px; horizontal-align: 0.5;} element-icon {size: 0px;} message {padding: 4px 14px 6px;} textbox {horizontal-align: 0.5;}')
    [[ -z "$THEME" ]] && exit 0
